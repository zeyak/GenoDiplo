...
rule setup_nr_db:  #FIX how to actuvste this before running blastn
    output:
        outdir = protected(directory(config["databases_dir"]))
    conda:
        "envs/genomics.yaml"
    script:
...
rule blastn:
    ...
    params:
        ...
        threads=32,
        evalue=1e-10,
        db_prefix=config["databases_dir"] + "/{db}"
    conda:
        "envs/genomics.yaml"
    script:
...
rule multiqc:
    ...
    conda:
         "envs/genomics.yaml"
    shell:
         'multiqc results/Genomics/1_Assembly/ -o {output.out_dir}'
